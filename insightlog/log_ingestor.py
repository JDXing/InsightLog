...
Parses with regex -> structured dict -> D1
"""
...
EVENT_PATTERNS = {
    ...
    "sudo_cmd":      re.compile(r'(?P<user>\S+)\s*:.*COMMAND=(?P<command>.+)$'),
    "su_failed":     re.compile(r'(?:FAILED su|authentication failure).*user=(?P<user>\S+)'),
...
def parse_line(line: str, source: str) -> Optional[Dict]:
    ...
    return {
        "timestamp":   ts,
        "source_file": source,
        "log_type":    "auth" if any(x in source for x in ["auth", "secure"]) else "syslog",
        "raw_line":    line,
        "host":        g.get("host", ""),
...
class LogTailer(threading.Thread):
    """Tails a log file; pushes new entries to D1 and calls on_new_log(log, id)."""
